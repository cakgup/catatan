--- /mnt/data/README_AOS-CX_updated.md	2026-07-14 04:07:02.566768916 +0000
+++ /mnt/data/README_AOS-CX_pemula.md	2026-07-14 04:08:07.287922750 +0000
@@ -1,145 +1,491 @@
 # AOS-CX Lab Guides
 
-## Ini sebenarnya belajar tentang apa?
...
-Rangkaian lab ini membahas cara **membangun, mengamankan, membuat redundan, dan melakukan troubleshooting jaringan enterprise/data center menggunakan Aruba AOS-CX**.
+Materi disusun bertahap agar pembaca pemula tidak hanya menyalin perintah, tetapi juga memahami:
 
-Materi dimulai dari fondasi **switching Layer 2**, kemudian berkembang ke **kontrol akses endpoint**, **high availability**, **dynamic routing**, hingga arsitektur jaringan modern berbasis **VXLAN EVPN**.
...
-Secara sederhana, perjalanan belajarnya adalah:
...
+Bayangkan sebuah kantor atau data center memiliki banyak komputer, server, dan switch. Agar semuanya dapat berkomunikasi dengan aman dan tetap tersedia ketika terjadi gangguan, jaringan memerlukan beberapa kemampuan berikut:
 ...
 ```text
-Perangkat pengguna terhubung ke switch
+Komputer atau server terhubung ke switch
         ↓
-MAC address dikenali dan perangkat mendapat role/VLAN
+Switch mengenali perangkat dan menempatkannya ke VLAN
         ↓
-STP mencegah loop pada jaringan Layer 2
+STP mencegah loop pada jalur Layer 2 yang redundan
         ↓
-VSX menyediakan redundansi switch dan link
+VSX menjaga layanan tetap berjalan jika link atau switch gagal
         ↓
-OSPF menyediakan routing otomatis antarjaringan
+OSPF membuat perangkat Layer 3 saling mempelajari rute
         ↓
-VXLAN membawa segmen Layer 2 melalui jaringan Layer 3
+VXLAN membawa jaringan Layer 2 melewati jaringan Layer 3
         ↓
-BGP EVPN mendistribusikan informasi endpoint secara dinamis
...
-| Modul | Sebenarnya belajar apa? |
+Pembaca tidak harus sudah mahir networking, tetapi sebaiknya mengenal istilah berikut.
+
+| Istilah | Penjelasan sederhana |
 |---|---|
-| **Spanning Tree** | Membuat jaringan Layer 2 memiliki jalur cadangan tanpa menimbulkan loop dan broadcast storm. |
-| **Local MAC Match Authentication** | Mengenali endpoint berdasarkan MAC address, kemudian memberikan role, VLAN, atau atribut tertentu. |
-| **VSX** | Membuat dua switch bekerja sebagai pasangan redundan menggunakan ISL, keepalive, sinkronisasi konfigurasi, dan multi-chassis LAG. |
-| **OSPF Dasar** | Membuat Layer 3 switch saling mempelajari rute secara otomatis tanpa menulis static route satu per satu. |
-| **OSPF Areas** | Membagi jaringan OSPF yang besar menjadi beberapa area serta memahami ABR, ASBR, redistribution, stub, dan NSSA. |
-| **Static VXLAN** | Memperluas jaringan Layer 2/VLAN melewati jaringan Layer 3 menggunakan tunnel VXLAN yang dikonfigurasi manual. |
-| **VXLAN EVPN** | Menggunakan BGP EVPN sebagai control plane untuk mengelola informasi MAC, VNI, VTEP, dan endpoint secara dinamis. |
-
-### Hasil akhir yang diharapkan
-
-Setelah menyelesaikan seluruh lab, Anda diharapkan mampu:
-
-- memahami perbedaan jaringan Layer 2, Layer 3, underlay, dan overlay;
-- membaca topologi, tabel MAC, tabel routing, serta status protokol;
-- membentuk dan memeriksa neighbor OSPF maupun BGP EVPN;
-- membangun redundansi menggunakan STP, LACP, dan VSX;
-- membangun VXLAN statis dan VXLAN berbasis EVPN;
-- melakukan troubleshooting secara berurutan dari interface sampai konektivitas ujung ke ujung.
...
-> **Inti pembelajaran:** membangun jaringan Aruba AOS-CX dari switching dasar hingga fabric data center VXLAN EVPN, sekaligus memahami alasan setiap konfigurasi dibuat.
+---
...
-- Download PNETLab: [https://pnetlab.com/pages/download](https://pnetlab.com/pages/download)
-- Siapkan image/OVA **Aruba AOS-CX Switch Simulator** yang sesuai.
-- Pastikan node AOS-CX dapat menyala dan console dapat diakses sebelum memulai modul.
-- Beberapa lab juga menggunakan **VPCS** sebagai host pengujian.
+### Unduh PNETLab
